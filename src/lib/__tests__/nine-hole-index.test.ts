...
describe("the index is converted to the holes being played", () => {
  it("halves an 18-hole index for a nine-hole round", () => {
    const m = courseHandicapMap([{ id: "p", handicap: 20, flightTeeId: null }], NEUTRAL, "t", 9, "own");
    expect(m.get("p")).toBe(10);
  });

  it("keeps a stored 9-hole index as-is for a nine-hole round", () => {
    const m = courseHandicapMap([{ id: "p", handicap: 7, handicapType: "9", flightTeeId: null }], NEUTRAL, "t", 9, "own");
    expect(m.get("p")).toBe(7);
  });

  it("doubles a stored 9-hole index for an eighteen-hole round", () => {
    const m = courseHandicapMap([{ id: "p", handicap: 7, handicapType: "9", flightTeeId: null }], NEUTRAL, "t", 18, "own");
    expect(m.get("p")).toBe(14);
  });

  it("leaves the ordinary case — 18-hole index, 18-hole round — alone", () => {
    const m = courseHandicapMap([{ id: "p", handicap: 14, flightTeeId: null }], NEUTRAL, "t", 18, "own");
    expect(m.get("p")).toBe(14);
  });
  ...
  it("applies slope to the halved index, not the full one", () => {
    ...
    const m = courseHandicapMap([{ id: "p", handicap: 20, flightTeeId: null }], hard, "t", 9, "own");
  ...
  it("treats an unrated nine-hole round the same way", () => {
    ...
    const m = courseHandicapMap([{ id: "p", handicap: 18, flightTeeId: null }], new Map(), null, 9, "own");
