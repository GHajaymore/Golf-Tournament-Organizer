 ...
 *
 * MOVED to `./country` on 2026-09-15, and re-exported here so that every
 * existing importer keeps working. It went because `org-profile.ts` needed it
 * too and that module had no imports at all — reaching into the course
 * directory for one function would have pulled `venue`, `scorecard-parse` and
 * `handicap` into everything that asks what kind of outfit a club is.
 *
 * The reasoning that produced it is unchanged and now lives beside it: only
 * names actually observed are mapped, and anything unrecognised comes back
